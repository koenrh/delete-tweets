...
class TweetReader(object):
    def __init__(
        ...
        self.since_date = (
            ...
            else parser.parse(since_date, ignoretz=True)
        )
        self.until_date = (
            datetime.now()
            if until_date is None
            else parser.parse(until_date, ignoretz=True)
        )
...
def delete(
    tweetjs_path, since_date, until_date, filters, s, min_l, min_r, dry_run=False
):
    with io.open(tweetjs_path, mode="r", encoding="utf-8") as tweetjs_file:
        ...
        for row in TweetReader(
            tweets, since_date, until_date, filters, s, min_l, min_r
        ).read():
